...
#
# What it does:
#   1. Bumps the trailing version segment in PVPQueueTime.toc
#      (1.0.N → 1.0.(N+1)). Done first so a malformed .toc fails
#      fast before anything destructive happens to the live install.
#   2. Wipes /Applications/World of Warcraft/_retail_/Interface/AddOns/PVPQueueTime
...
#   package-addon.sh  — sibling packager script
#
# Kept:
...
DEST="/Applications/World of Warcraft/_retail_/Interface/AddOns/PVPQueueTime"
...
# Only the trailing numeric segment is bumped (1.0.N -> 1.0.(N+1)).
# The prefix (1.0) is preserved — bump it manually for larger releases.
